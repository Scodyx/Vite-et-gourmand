...
public class OrderController {
 ...
 @PatchMapping("/api/v1/users/me/orders/{id}/cancel") View cancelAlias(Authentication a,@PathVariable Long id,@Valid @RequestBody Cancellation r){
  return service.cancelMine(a.getName(),id,r);}
 @GetMapping("/api/v1/employee/orders") Page<View> all(@PageableDefault(size=50) Pageable p){return service.all(p);}
 @GetMapping("/api/v1/employee/orders/{id}") Detail detail(@PathVariable Long id){return service.employeeDetail(id);}
