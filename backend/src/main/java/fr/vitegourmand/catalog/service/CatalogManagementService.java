...
public class CatalogManagementService {
    ...
    public CatalogManagementService(DishRepository dishes, AllergenRepository allergens, OpeningHoursRepository hours) {
        this.dishes = dishes; this.allergens = allergens; this.hours = hours;
    }

    @Transactional(readOnly = true)
    public List<AllergenView> allergens() { return allergens.findAll().stream().map(this::view).toList(); }
    @Transactional(readOnly = true)
    public List<AdminAllergenView> adminAllergens() { return allergens.findAll().stream().map(this::adminView).toList(); }
    @Transactional(readOnly = true)
    public AdminAllergenView allergen(Long id) { return adminView(allergenEntity(id)); }

    @Transactional
    public AllergenView createAllergen(AllergenInput request) {
        ensureUniqueAllergen(request.name(), null);
        var allergen = new Allergen(); allergen.setName(cleanName(request.name()));
        return view(allergens.save(allergen));
    }

    @Transactional
    public AllergenView updateAllergen(Long id, AllergenInput request) {
        var allergen = allergens.findById(id).orElseThrow(() -> new NotFoundException("Allergène introuvable"));
        ensureUniqueAllergen(request.name(), id); allergen.setName(cleanName(request.name())); return view(allergen);
    }

    @Transactional(readOnly = true)
    public DishAllergensView dishAllergens(Long dishId) { return allergensView(dishEntity(dishId)); }
    @Transactional
    public DishAllergensView addAllergen(Long dishId, Long allergenId) {
        var dish=dishEntity(dishId);var allergen=allergenEntity(allergenId);
        if(!dish.getAllergens().add(allergen))throw new BusinessException("Cet allergène est déjà associé au plat");
        return allergensView(dish);
    }
    @Transactional
    public DishAllergensView removeAllergen(Long dishId, Long allergenId) {
        var dish=dishEntity(dishId);var allergen=allergenEntity(allergenId);
        if(!dish.getAllergens().remove(allergen))throw new BusinessException("Cet allergène n'est pas associé au plat");
        return allergensView(dish);
    }
    @Transactional
    public DishAllergensView replaceAllergens(Long dishId, AllergenIds request) {
        ...
        dish.getAllergens().clear();dish.getAllergens().addAll(replacements);
        return allergensView(dish);
    }

    @Transactional(readOnly = true)
    public List<DishView> dishes() {
        Map<Long, Long> menuCounts = dishes.countMenusByDish().stream()
                .collect(Collectors.toMap(DishRepository.MenuCount::getDishId, DishRepository.MenuCount::getMenuCount));
        return dishes.findAll().stream().map(value -> view(value, menuCounts.getOrDefault(value.getId(), 0L))).toList();
    }

    @Transactional(readOnly = true)
    public DishView dish(Long id) { return view(dishEntity(id)); }

    @Transactional
    public DishView createDish(DishInput request) {
        ensureUniqueDish(request.name(), null);
        var dish = new Dish(); apply(dish, request); return view(dishes.save(dish));
    }

    @Transactional
    public DishView updateDish(Long id, DishInput request) {
        var dish = dishEntity(id); apply(dish, request); return view(dish);
    }

    @Transactional
    public DishView enableDish(Long id, boolean value) {
        var dish = dishEntity(id); dish.setActive(value); return view(dish);
    }

    @Transactional(readOnly = true)
    public List<HoursView> hours() { return hours.findAllByOrderByDisplayOrderAsc().stream().map(this::view).toList(); }
    ...
    public HoursView updateHours(Long id, HoursInput request) {
        ...
        hours.findByDayOfWeek(request.dayOfWeek()).filter(other -> !other.getId().equals(id))
                .ifPresent(other -> { throw new BusinessException("Ce jour existe déjà"); });
        value.setDayOfWeek(request.dayOfWeek()); value.setClosed(request.closed());
        value.setOpeningTime(request.closed() ? null : request.openingTime());
        value.setClosingTime(request.closed() ? null : request.closingTime());
        value.setDisplayOrder(request.displayOrder()); return view(value);
    }
    ...
    private void apply(Dish dish, DishInput request) {
        ...
        dish.setType(request.type()); dish.setActive(request.active());
    ...
    private void ensureUniqueDish(String name, Long id) {
        ...
        dishes.findByNameIgnoreCase(normalized).filter(other -> !other.getId().equals(id))
                .ifPresent(other -> { throw new BusinessException("Ce nom de plat existe déjà"); });
    }

    private void validate(HoursInput request) {
        if (!request.closed() && (request.openingTime() == null || request.closingTime() == null
    ...
    private void ensureUniqueAllergen(String name, Long id) {
        ...
        allergens.findByNameIgnoreCase(cleaned).filter(value -> !value.getId().equals(id))
                .ifPresent(value -> { throw new BusinessException("Cet allergène existe déjà"); });
    }
    ...
    private AllergenView view(Allergen value) { return new AllergenView(value.getId(), value.getName()); }
    private AdminAllergenView adminView(Allergen value){return new AdminAllergenView(value.getId(),value.getName(),allergens.countDishesByAllergenId(value.getId()));}
    private DishAllergensView allergensView(Dish dish){
        var values=dish.getAllergens().stream().sorted(java.util.Comparator.comparing(Allergen::getName)).map(this::view).toList();
    ...
    private DishView view(Dish value, long menuCount) {
        return new DishView(value.getId(), value.getName(), value.getDescription(), value.getType(),
                value.isActive(), value.getAllergens().stream().map(this::view).toList(),
                menuCount);
    }
    private HoursView view(OpeningHours value) {
        return new HoursView(value.getId(), value.getDayOfWeek(), value.getOpeningTime(),
                value.getClosingTime(), value.isClosed(), value.getDisplayOrder());
    }
}
