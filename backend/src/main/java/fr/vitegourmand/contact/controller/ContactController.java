...
public class ContactController {
 private final ContactRequestRepository repository; private final JavaMailSender mail; private final String recipient; private final String from;
 public ContactController(ContactRequestRepository repository,JavaMailSender mail,@Value("${app.contact-email}")String recipient,
  ...
  this.repository=repository;this.mail=mail;this.recipient=recipient;this.from=from;
 }
 ...
 public void send(@Valid @RequestBody ContactRequestDto dto){
  ...
  var message=new SimpleMailMessage();message.setFrom(from);message.setTo(recipient);message.setReplyTo(dto.email());message.setSubject("[Contact V&G] "+dto.title());message.setText(dto.message());mail.send(message);
 }
}
