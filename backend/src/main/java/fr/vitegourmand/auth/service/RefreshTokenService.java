...
public class RefreshTokenService {
 ...
 @Transactional public Session rotate(String raw){
  ...
  if(!current.getUser().isEnabled())throw new BusinessException("Compte désactivé");current.revoke();return session(current.getUser());
 }
 ...
 private User user(String email){return users.findByEmailIgnoreCase(email.trim()).filter(User::isEnabled).orElseThrow(()->new BusinessException("Identifiants invalides"));}
